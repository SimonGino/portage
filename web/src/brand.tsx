...
//
// 一条 path、`fill="currentColor"`：颜色跟着上下文走（也就是 --ink），不为标识单开色值，
// 换配色时不必回来复查这里（DESIGN.md §3 的单色约束对标识同样成立）。
//
// 这份是**页面内**用的。favicon 是另一份 `web/public/portage-mark-v2.svg`：那份是独立文档，
// currentColor 在里面没有上下文可继承，两个色值只能写死在 SVG 自己的 <style> 里，按**标签栏**
// 的深浅挑——管理端页面只做亮色（DESIGN.md §1.5 / v0.29），标签栏是另一回事，见 §11。
// 两处的 path 数据必须一致，改一处记得改另一处。设计源在 Claude Design 项目
// 「Portage Logo Design Brief」的 assets/mark-hull.svg（见 issue #46）。

/** MARK_PATH 与 web/public/portage-mark-v2.svg 里的那条必须逐字一致。 */
