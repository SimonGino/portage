...
go build ./cmd/portage    # 不带 -tags webui 也能过，/panel 出「前端未构建」页——CI 与无 Node 的机器走这条
...
- **管理端弹框（Dialog/Confirm）走弹框档**：一律用 `web/src/ui.tsx` 的 `Dialog` / `Confirm` 组件；标题、头部、按钮与文案的规格以 `DESIGN.md` §2 表面清单为准，页面级字号与按钮档不进框。
