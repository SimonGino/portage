...
**思考 token**：
usage 里的 reasoning token（v0.66）——是 **output token 的明细而非另一笔加数**（`total = input + output` 恒成立，思考不进这个加法），同 `毛值 input` 下缓存两项与 input 的关系。流水那一列**三档**：正数 = 思考了这么多，`0` = 上游报了、这次没思考，`NULL` = 上游这一轮根本不报（没开思考的 Anthropic 响应连 `output_tokens_details` 容器都不发、CC 挂非推理模型、v0.66 之前的老行）。**载体三家各不同形**（v0.79）：CC 是 `completion_tokens_details.reasoning_tokens`，Responses 是 `output_tokens_details.reasoning_tokens`，Anthropic 是 `output_tokens_details.thinking_tokens`——容器名 A 与 R 同、字段名 CC 与 R 同，抄任何一边都对不上。
_Avoid_: 把它加进 total 或从 output 里减掉；把 NULL 抹成 0（那是在说「确凿零思考成本」）；说「Anthropic 一路 NULL」（v0.66 原文如此，v0.79 由自己的字节证伪）
