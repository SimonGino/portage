...
type Summary struct {
	...
	// 这个数」——上游整个 details 容器不发时（老式兼容上游、中转裁剪、CC 挂非推理
	// 模型）就是这一档。**「不带思考的调用」落哪一档随上游而定，不预设形态**（口径层
	// v0.80）：本库六份非思考 anthropic 转录一份都不带这个容器（落没报），而 #5 现场
	// 见过带容器且 thinking_tokens 为 0 的（落报了 0）。判据一律是键在不在。只看
