...
**实现时须同批改掉的三处过期注释**：仓库根 `config.yaml`（gitignore 覆盖，本机那份）与 `deploy/config.docker.yaml`，头部都写着「业务配置……全部落 DB，不写这里」，那句在挂了文件的形态下是假的；外加 `cmd/portage/main.go:54-56` 空 `api_keys` 那条警告的注释（见上，口径层 v0.92 之后它只写了两侧中的一侧）。**本批都不改**——它们是运行中的产物，今天那几句话还是真的，现在改是另一个方向的撒谎。
