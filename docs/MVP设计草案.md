...
`conversionOpen` 也因此换了职责：它不再是「逐格放开」的临时闸，而只挡一件事——**没有上游对应端点的入口端点**。那就是 `/v1/messages/count_tokens`：它与 `/v1/messages` 的入口协议同为 anthropic，命中非 anthropic 渠道时只能回 501，这不是「还没做」而是没得做。判据必须按端点而不按协议，原因即在此。
...
>
> **v0.33 追加**：未停用渠道的 `protocols` 必须非空且逐项合法（`store.checkChannelFields` 调 `protocol.ParseSet`）。空集合选不出出站协议，每次请求才 500——同属 v0.21 通则要拦的形态。`count_tokens` 不需要额外的闸：`conversionOpen` 的判据是**端点**而非协议，渠道不说 `anthropic` 时它必然落进「没有对应的转换路径」回 501，不会被回退顺序送去 `/v1/chat/completions`。
...
- **CC 入口的 `image_url.detail` 静默丢弃。** `parseCCImage` 只取 `url`，`detail`（`low`/`high`/`auto`，影响上游的切图与计费）既没进 `Image`、也没进 `Extras`、更没登记丢弃项——与本项目「丢得有声音」的一贯口径不合。Responses 那边同名字段反而留在 `Extras` 里（`collectExtras` 只排除 `type`/`image_url`/`file_id`），两个入口对同一件事不一致。CC→R 方向本来是搬得过去的（Responses 的 `input_image` 有 `detail`）。**不在本票范围**，#1 的清单一个字都没提它；交 PO 决定是补转换还是补一个丢弃项。
