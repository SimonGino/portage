...
| GET | `/admin/api/usage?days=&by=model\|key\|credential&from=&to=` | 汇总，`by` 选维度：按模型（默认）、按**网关 API Key**（v0.53）或按**上游凭证**（v0.35）。后两者是两件事，标签写全称——只写「按凭证」两边都像。`from`/`to` 是 **unix 秒的半开区间 `[from, to)`**（v0.92，口径层 v0.86），两个都给时**顶掉 `days`**、`windowStart` 不参与，只给一个当没给；排行页点中节律带上某一格之后，排行列表 / 环形图 / 堆叠条用它重取那一格的构成。**解析失败回 400**，见 §8.2 |
...
- **`from`/`to` 解析失败回 400，不走「认不得就忽略」**：先指出冲突再执行——`clampQuery` 与 `by` 的既有立论是「写错了不该让整个页面打不开」，那条管的是**只影响展示**的参数；`from`/`to` 定的是**记账范围**，静默忽略会拿整窗的合计去回答「这一小时」，那不是显示得不对，是**数字本身错了且看不出来**。例外只开给这两个参数，`days` / `by` / `unit` 照旧忽略。
