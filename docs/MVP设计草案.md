...
> 状态：草案 v0.88
> v0.88 变更（[#14](https://github.com/SimonGino/portage/issues/14) 旧仓 issue 号清理，2026-08-18）：**只改引用形态，`internal/` 一行未动，口径与技术结论一字未动。**①本文 283 处 `#N` 逐处判类：指新仓票的保留；旧仓票旁边已有 `§X.Y` / `vX.YY` 锚点的删数字（12 处，含 §7.5/§7.6/§7.7 与 §4.6 四个小节标题——标题里的号删掉后 markdown 自动锚点随之变化，全仓无交叉链接引用这几个锚点，已查）；其余旧仓票限定成 `portage-legacy#N`。**文首的版本记录一律限定不删号**，那个号本身就是溯源证据。②旧仓 `portage-legacy#33`/`portage-legacy#37`/`portage-legacy#98` 是 transfer 号，改成新仓 `#1`/`#2`/`#3`；§4.6 标题因此出现同一张票被写成两个号的重复，合并成一个。③外部仓引用（`openai/codex#7782`、`opencodex #422`、`sub2api #3887`）与 §0 占位假设编号不是 issue 引用，原样保留。修改人 jinpenga。
