...
`blocked` 与 `ready-for-agent` 互斥：规格再全，前置票没落地就不能说「AI 可独立领走」。前置票一关，摘 `blocked` 换 `ready-for-agent`。M2 的转换路径是链式依赖（portage-legacy#10 → portage-legacy#11 → portage-legacy#12），这个状态会反复用到。
